...
function MerchCard({ label, price, photo, note, sizes, large }) {
  return (
    <div className='rounded-lg overflow-hidden border border-slate-200 bg-white shadow-sm'>
      {photo ? (
        <img src={photo} alt={label} className={`w-full object-cover ${large ? 'h-56 sm:h-72' : 'h-40 sm:h-48'}`} />
      ) : (
        <div
...
function SmallItem({ label, price, photo }) {
  return (
    <div className='flex items-center gap-3 py-2 border-b border-slate-100 last:border-0 pr-4'>
      {photo ? (
        <img src={photo} alt={label} className='w-10 h-10 rounded object-cover shrink-0' />
      ) : (
  ...
  return (
    <div className='max-w-3xl mx-auto p-4 pb-12'>
      ...
      <div className='grid grid-cols-1 sm:grid-cols-3 gap-4'>
        {CLOTHING.map((item) => (
          <MerchCard key={item.label} large {...item} />
        ))}
      </div>
