...
export default function Weather() {
  ...
  // Load data, re-fetch when connectivity returns, listen for SW background-sync updates
  React.useEffect(() => {
    load()
    window.addEventListener('online', load)
    // Background sync: SW refreshes cache when connectivity returns even with app closed
  ...
  // Check whether periodic background sync is available on this browser
  React.useEffect(() => {
    navigator.serviceWorker?.ready.then((reg) => setSyncSupported('periodicSync' in reg))
  }, [])

  // Register or unregister periodic sync when the user preference changes
  React.useEffect(() => {
